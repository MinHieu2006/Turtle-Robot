(function() {
    ...
    var requestQueue = []; // Hàng đợi các yêu cầu
    var isProcessing = false; // Trạng thái xử lý
    ...
    function processQueue() {
        // Nếu đang xử lý hoặc hàng đợi trống, không làm gì
        if (isProcessing || requestQueue.length === 0) return;
    
        isProcessing = true; // Đặt trạng thái đang xử lý
        const currentRequest = requestQueue.shift(); // Lấy yêu cầu đầu tiên trong hàng đợi
